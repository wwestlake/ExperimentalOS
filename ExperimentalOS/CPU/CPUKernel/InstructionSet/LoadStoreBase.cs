...
namespace LagDaemon.ExperimentalOS.CPU.CPUKernel.InstructionSet
{
    ...
    internal abstract class LoadStoreBase : Instruction
    {
        ...
        internal LoadStoreBase(InstructionCodes code, int r1, int r2, int value, int address, string comment)
            : base(code, comment)
        {
            this.r1 = r1;
            this.r2 = r2;
            ...
            this.value = value;
            if (r2 != Symbols.NullRegister && address != (int)Symbols.NullAddress)
            {
            ...
            else if (r2 == Symbols.NullRegister && address == (int)Symbols.NullAddress)
            {
                this.Size = 3 + sizeof(int);
            }
        }

        internal LoadStoreBase(InstructionCodes code, int r1, int r2, int value, int address) : this(code, r1, r2, value, address, string.Empty) { }
        ...
        protected override Instruction Assemble(IInstructionFactory factory, string assemblyLine)
        {
            ...
            if (line.StartsWith(Code.ToString().ToLower()))
            {
                if (m1.Success)
                {
                    return NewInstruction(factory, int.Parse(m1.Groups["r1"].Value), int.Parse(m1.Groups["r2"].Value), 0, int.Parse(m1.Groups["address"].Value), m1.Groups["comment"] != null ? m1.Groups["comment"].Value : string.Empty);
                }
                if (m2.Success)
                {
                    return NewInstruction(factory, int.Parse(m2.Groups["r1"].Value), int.Parse(m2.Groups["r2"].Value), 0, (int)Symbols.NullAddress, m2.Groups["comment"] != null ? m2.Groups["comment"].Value : string.Empty);
                }
                if (m3.Success)
                {
                    return NewInstruction(factory, int.Parse(m3.Groups["r1"].Value), Symbols.NullRegister, 0, int.Parse(m3.Groups["address"].Value), m3.Groups["comment"] != null ? m3.Groups["comment"].Value : string.Empty);
                }
                if (m4.Success)
                {
                    return NewInstruction(factory, int.Parse(m4.Groups["r1"].Value), Symbols.NullRegister, int.Parse(m4.Groups["value"].Value), (int)Symbols.NullAddress, m4.Groups["comment"] != null ? m4.Groups["comment"].Value : string.Empty);
                }
            }
            throw new InstructionParseException("Incorrect op code for this class: {0}, {1}", this.GetType().FullName, assemblyLine);
        }

        protected abstract Instruction NewInstruction(IInstructionFactory factory, int r1, int r2, int value, int address, string comment);
